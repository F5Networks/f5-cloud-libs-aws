        ...
        .then((autoscalingGroups) => {
            if (autoscalingGroups) {
                autoscalingGroups.forEach((autoscaleGroup) => {
                    ...
                    for (let i = 0; i < autoscaleGroup.Instances.length; ++i) {
                        const instanceId = autoscaleGroup.Instances[i].InstanceId;
                        this.instanceIdToAutoscaleGroupMap[instanceId] =
                            autoscaleGroup.AutoScalingGroupName;
                        this.instanceIdToLaunchConfigMap[instanceId] =
                            autoscaleGroup.Instances[i].LaunchConfigurationName;
                    }
                });
            }
