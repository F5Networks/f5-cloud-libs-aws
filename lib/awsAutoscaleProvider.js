            ...
            this.s3.listObjectsV2(params).promise()
                .then(function(data) {
                    if (data.KeyCount !== 0) {
                        ...
                        var createBackupFolder = function() {
                            var backupDeferred = q.defer();
                            putObject(this.s3, this.providerOptions.s3Bucket, BACKUP_FOLDER)
                                .then(function() {
                                    backupDeferred.resolve();
                                })
                                .catch(function(err) {
                                    backupDeferred.reject(err);
                                });
                            return backupDeferred.promise;
                        };

                        cloudUtil.tryUntil(this, cloudUtil.DEFAULT_RETRY, createBackupFolder)
                            .then(function() {
                ...
                .catch(function(err) {
                    logger.warn('Error checking for backup folder');
                    deferred.reject(err);
                });
    ...
    var getAndValidateCredentials = function() {
        ...
        return getObject(this.s3, this.providerOptions.s3Bucket, CREDENTIALS_KEY)
            .then(function(data) {
                ...
                masterBigIp = new BigIp({logger: logger});
...
AwsAutoscaleProvider.prototype.putMasterCredentials = function() {
    ...
    return cloudUtil.tryUntil(this, cloudUtil.DEFAULT_RETRY, putCredentials);
};
