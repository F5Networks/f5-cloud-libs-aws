...
const CREDENTIALS_KEY = "master_credentials";
...
AwsAutoscaleProvider.prototype.putInstance = function(instance) {
    ...
    putObject(this.s3, this.providerOptions.s3Bucket, this.nodeProperties.instanceId, JSON.stringify(instance))
        .then(function(data) {
    ...
    s3.listObjectsV2(params).promise()
        .then(function(data) {
            ...
            data.Contents.forEach(function (element) {
                if (element.Key !== CREDENTIALS_KEY) {
...
var getStoredInstance = function(s3, s3Bucket, instanceId) {
    var deferred = q.defer();

    getObject(s3,  s3Bucket, instanceId)
        .then(function(data) {
            var parsed;
