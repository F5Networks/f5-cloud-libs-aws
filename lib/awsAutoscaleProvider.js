...
AwsAutoscaleProvider.prototype.init = function init(providerOptions, options) {
    ...
    this.launchConfigName = '';
    this.launchConfigMap = {};
...
AwsAutoscaleProvider.prototype.getInstances = function getInstances(options) {
    ...
    getAutoscalingGroupInstances(this.autoscaling, this.nodeProperties.autoscaleGroupId)
        .then((data) => {
            ...
            const instancesToConsider = filterInstances(autoscalingGroup.Instances);
            ...
            if (externalTag) {
                return this.getVmsByTag(externalTag);
            }

            this.launchConfigName = autoscalingGroup.LaunchConfigurationName;
 ...
 *                   }
 */
AwsAutoscaleProvider.prototype.getVmsByTag = function getVmsByTag(tag) {
    const deferred = q.defer();
    const vms = [];
...
AwsAutoscaleProvider.prototype.electMaster = function electMaster(instances) {
    ...
    const canInstanceBeElected = function (instance, instanceId) {
        if (
            instance.versionOk &&
            instance.providerVisible &&
            this.launchConfigMap[instanceId] === this.launchConfigName
        ) {
            return true;
        }
        return false;
...
/**
 * Gets the instances AWS sees in the autoscaling group
 *
 * @param {Object} autoscaling - Aws.autoscaling instance
 * @param {String} autoscaleGroupId - Autoscale group Id
 */
function getAutoscalingGroupInstances(autoscaling, autoscaleGroupId) {
    const deferred = q.defer();
    const params = {
        AutoScalingGroupNames: [autoscaleGroupId]
    };
