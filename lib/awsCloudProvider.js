 ...
 *                    elected master.
 */
AwsCloudProvider.prototype.electMaster = function electMaster(instances) {
    ...
    const isLowestIpWithRunningConfig = instancesWithRunningConfig.some((instanceWithRunConf) => {
        return masterId === instanceWithRunConf.id;
    });
    ...
    if (!isLowestIpWithRunningConfig && instancesWithRunningConfig.length > 0) {
        ...
        masterId = instancesWithRunningConfig[0].id;
    }
 ...
 *                    }
 */
AwsCloudProvider.prototype.getMasterCredentials = function getMasterCredentials(mgmtIp, mgmtPort) {
 ...
 */
AwsCloudProvider.prototype.isValidMaster = function isValidMaster() {
    return q(true);
};
 ...
 */
AwsCloudProvider.prototype.masterElected = function masterElected(instanceId) {
    let promise;
...
/**
 * Tags the Master Instance.
 * Removes the Master Instance tag from instances that are not the Master.
 *
 * @param {String} masterId - ID of the Master Instance.
 * @param {Object} instances - Dictionary of instances as returned by getInstances.
 *
 * @returns {Promise} A promise which will be resolved when the operation
 *                    is complete
 */
AwsCloudProvider.prototype.tagMasterInstance = function tagMasterInstance(masterId, instances) {
    const deferred = q.defer();
    let clusterTagKey;

    if (!masterId) {
        deferred.reject(new Error('MasterId must be provided'));
        return deferred.promise;
    }

    getTagsFromInstance(this.ec2, masterId)
        .then((tags) => {
        ...
        .then((stackName) => {
            ...
            clusterTagKey = `${stackName}-master`;
            const params = {
                Resources: [masterId],
                Tags: [
                    {
            ...
            logger.silly('Adding master tag to instance: ', masterId);
            return this.ec2.createTags(params).promise()
                .catch((err) => {
                    q.reject(new Error('Unable to tag master instance', err));
                });
        })
        .then(() => {
            return cleanUpMasterTags(this.ec2, masterId, instances, clusterTagKey)
                .then(() => {
                    deferred.resolve();
                })
                .catch((err) => {
                    return q.reject(new Error('Unable to clean-up master tags', err));
                });
        })
        .catch((err) => {
            logger.warn(err);
 ...
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
AwsCloudProvider.prototype.masterInvalidated = function masterInvalidated(instanceId) {
 ...
 *                    is complete
 */
AwsCloudProvider.prototype.putMasterCredentials = function putMasterCredentials() {
    const deferred = q.defer();
 ...
 *
 */
AwsCloudProvider.prototype.getMasterStatus = function getMasterStatus() {
...
/**
 * Removes tags from instances that are not the Master Instance
 *
 * @param {Object} ec2 - AWS EC2 instance
 * @param {String} masterId - Instance ID of the Master instance
 * @param {Object} instances - Dictionary of instances as returned by getInstances
 * @param {String} tagKey - Key Name of the cluster Tag
 *
 * @returns {Promise} - Returns a Promise
 */
function cleanUpMasterTags(ec2, masterId, instances, tagKey) {
    ...
    const deleteInstanceTags = function deleteInstanceTags(instanceId) {
        if (instanceId !== masterId) {
            ...
            logger.silly('Deleting master tag from instance: ', instanceId);
